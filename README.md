...
- `templates/` のテンプレートを編集して見た目を変えられます（未実装）。
- `build.js` を改造してタグ、カテゴリ、ページネーション等を追加できます。

## カスタムドメイン

- GitHub リポジトリの Pages 設定でカスタムドメインを登録してください。
- カスタムドメイン利用の際は `site.config.json` の `baseUrl` の値は、横渡リンクの生成処理に不備が発生するため空文字 `""` としてください。
